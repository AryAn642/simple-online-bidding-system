...
$high = $conn->query("SELECT bid_amount FROM bids where product_id =" .$_GET['id']." order by bid_amount desc limit 1 ");
$higha;
while ($highest = $high->fetch_assoc()) {
    $higha=$highest['bid_amount']."<br>";
}
...
<div class="container-fluid">
	...
	<p>Highest Bid: <b id="hbid"><?php echo $higha ?></b></p>
	<p>Description:</p>
    ...
    var _updateBid = setInterval(function(){
    	$.ajax({
    		...
    		success:function(resp){
    			if(resp && resp > 0){
    				$('#hbbid').text(parseFloat(resp).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
    			}
    		}
    	})
    },1000)
